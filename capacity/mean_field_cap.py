...
def manifold_analysis_corr(XtotT, kappa, n_t, t_vecs=None):
    ...
    for i in range(num_manifolds):
        ...
        if t_vecs is not None:
            a_Mfull, R_M, D_M, normv2, R_g, D_g = each_manifold_analysis_D1(sD1, kappa, n_t, t_vec=t_vecs[i])
        else:
            a_Mfull, R_M, D_M, normv2, R_g, D_g = each_manifold_analysis_D1(sD1, kappa, n_t)
    ...
    return a_Mfull_vec, R_M_vec, D_M_vec, 1/np.mean(normv2s), R_g, D_g
...
#                 print(s_f[-1, :])
    ...
    # Gaussian geometry
    R_g = np.sqrt(np.trace(ss[0:D,:].T@ss[0:D,:])/n_t)
    ss_norm = np.diag(ss.T@ss)
    D_g = np.sum(gg**2/ss_norm)/n_t

    return a_Mfull, R_M, D_M, f_all.mean(), R_g, D_g
