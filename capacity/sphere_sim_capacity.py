...
def sample_spheres(N, P, K, s, r, r0, lam ,psi): 
    ''' 
    Sample from the surface of spheres with rank one axis and centroid correlations. 
    Args: 
    - N: Ambient dimensionality
    - P: Number of manifolds
    - K: Intrinsic dimensionality of the spheres
    - s: number of samples per manifold
    - r: radius
    - r0: centroid norm
    - lam: axis correlation strength
    - psi: centroid correlation strength
    Returns: 
    - Array of shape (P, N, s) containing samples for each sphere 
    '''
    # sample sphere axes and centroids. Note that this function returns the centroids as the final axis
    axes = get_axes(N, P, K, r**2, r**2*lam, r0**2, r0**2*psi) 
    axes, centroids = axes[..., :-1], axes[..., -1]
    # sample from the surface by taking a unit-norm linear combination of the axes. The sample is taken randomly from the surface of the unit K-dimensional ball
    samps = np.random.randn(P, s, K)
    samps = samps / np.linalg.norm(samps, axis=2, keepdims=True) 
    # sum over the sphere axes. Obtain the samples in an array of shape (P, N, s)
    samps = np.einsum('mli, msi -> mls', axes, samps)
    # add in the centroids 
    samps = samps + centroids[..., np.newaxis]
    return samps, centroids, axes 